...
class ArrowManager {
  ...
  bool doesArrowPathIntersectTile(Arrow arrow, Rect tileBounds, Offset baseOffset) {
    ...
    if (effectiveSourceNode != null && effectiveTargetNode != null) {
      ...
      // Если хотя бы один из узлов пересекается с тайлом, это может быть важным
      // для правильной отрисовки стрелки, но не означает автоматически, что стрелка пересекает тайл
      // Мы продолжаем выполнение для проверки пересечения самого пути стрелки
    }
