...
class ArrowTileCoordinator {
  ...
  bool doesArrowIntersectTile(Arrow arrow, Rect tileBounds, Offset baseOffset) {
    // Сначала проверим, связаны ли узлы стрелки с невидимыми swimlane
    ...
    if (sourceNode != null && _isNodeHiddenInCollapsedSwimlane(sourceNode) ||
        targetNode != null && _isNodeHiddenInCollapsedSwimlane(targetNode)) {
      return false; // Не рисуем стрелку, если один из узлов скрыт
    }
  ...
  // Вспомогательный метод для проверки, является ли узел скрытым в свернутом swimlane
