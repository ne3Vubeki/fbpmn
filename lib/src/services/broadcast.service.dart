...
class Broadcast {
  ...
  Broadcast({required String app, required String source})
    : _app = app,
      _source = source,
      _element =
          document.querySelector('#q-wasm-container-fbpmn') as HTMLElement?;

  EventApp? register() {
    if (_element == null) {
      return null;
    }
    EventApp data = EventApp(app: _app, source: _source);
    _broadcastAppEvent(
      'qp_register_app_channel',
      createJSInteropWrapper(data),
    );
    return data;
  }

  EventApp emitAction({EventApp? event}) {
    EventApp data = event ?? EventApp(app: _app, source: _source);
