...
class NodeManager extends Manager {
  ...
  Future<void> _saveNodeToTiles() async {
    ...
    final manualCandidateEntries = state.manualLayoutTrainNeuralPolish
        ? selectedNodes
            .map((node) => (
                  node: node,
                  originPosition: _multiDragStartPositions[node.id],
                  candidatePosition: node.aPosition ?? (state.delta + node.position),
                ))
            .toList(growable: false)
        : const [];
    final manualMovedNodes = state.manualLayoutTrainNeuralPolish
        ? manualCandidateEntries
            .where((entry) =>
                entry.originPosition != null &&
                (entry.candidatePosition - entry.originPosition!).distance > 0.01)
            .toList(growable: false)
        : const [];
    ...
    if (manualMovedNodes.isNotEmpty) {
      final neuralPolishService = NeuralPolishService.withIndexedDb(
        state: state,
